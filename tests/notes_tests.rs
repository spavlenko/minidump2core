...
fn note_name_padded_to_4_bytes() {
    ...
    assert_eq!(note.bytes.len() % 4, 0, "total length must be 4-byte aligned");
...
fn note_already_aligned_no_extra_padding() {
    ...
    assert_eq!(note.bytes.len(), 12 + 4 + 4, "no extra padding should be added");
    assert_eq!(note.bytes.len() % 4, 0);
